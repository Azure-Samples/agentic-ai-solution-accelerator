...
- **Per-agent model override** → edit `accelerator.yaml` `models:` block (add a slug entry), then set `scenario.agents[].model: <slug>`. `scripts/sync-models-from-manifest.py` picks it up on the next `azd up` preprovision; Bicep provisions the extra deployment with the shared RAI policy; `foundry-bootstrap.py` re-points the Foundry agent. Lint rules `models_block_shape` + `agent_model_refs_exist` enforce shape. Omit the block for single-default back-compat.
