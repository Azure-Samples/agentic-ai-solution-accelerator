...
2. Read App Insights telemetry emitted by the workflow, and know which
   dashboard panels require partner-wired emitters to light up.
3. Edit an agent's instructions the supported way (spec file + `azd
   provision`), not by portal drift.
4. Scaffold a new side-effect tool via `/add-tool` with HITL baked in,
   and know why the redteam case is not optional.
5. Run the quality and redteam evals against your deployment and read
   `scripts/enforce-acceptance.py` output.
6. Swap the model via `accelerator.yaml → models[]`.
7. Scaffold a new scenario with `/scaffold-from-brief` and know what
...
## Lab 2 — Run the flagship and read telemetry

**Goal:** exercise the shipped scenario and correlate a request to
App Insights events.
...
3. Open `infra/dashboards/roi-kpis.json` in the repo, copy the JSON,
...
- Answer for yourself: why are both the "$ per call" and
  "Groundedness eval score trend" panels empty in the shipped
  flagship? (Hint: the workbook keys those panels off `cost.call`
  and `eval.result` custom events — see
  `infra/dashboards/roi-kpis.json`. Neither event is emitted by
  the shipped workflow: `src/accelerator_baseline/cost.py`
  exports `record_call_cost(...)` but nothing calls it, and
  `eval.result` is not wired in the shipped eval runners.
  `evals/quality/run.py` does synthesise `cost_usd` into
...
## Lab 3 — Edit an agent's instructions the supported way
...
## Lab 4 — Add a side-effect tool with `/add-tool`
...
3. Copilot generates `src/tools/<tool_name>.py` with HITL
   ...
   **Add the module-level `HITL_POLICY = "always"`
   line if the scaffold didn't emit it** — the lint rule below
   keys on it.
...
- Remove the `checkpoint(...)` call from the tool and re-run lint.
  ...
  before continuing.
- There is no lint rule that couples a newly-added tool to a
  specific redteam case, so the acceptance gate will *not* auto-fail
  on missing coverage. The contract is cultural: every side-effect
  tool ships with at least one redteam case under `evals/redteam/`.
  Review `evals/redteam/cases.jsonl` and add one for your new tool
  before you call the scaffold done.

---

## Lab 5 — Run evals and interpret acceptance
...
3. Re-run the eval chain from Lab 5. Quality may shift — that's the
   point. Note how cost (if you wired it) and latency move.
